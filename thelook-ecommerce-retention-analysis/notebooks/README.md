...
with product_sales as (
  -- subquery to calculate the total revenue, product quantity sold, and product cost for each product category
  select
    oi.product_id, -- The unique identifier of the product in the order_items table
    p.category, -- The category of the product from the products table
    sum(oi.sale_price) as total_revenue, -- The total revenue generated from the sales of the product
    count(oi.product_id) as product_bought, -- The total quantity of the product sold
    p.cost -- The cost of the product from the products table
  from
    `bigquery-public-data.thelook_ecommerce.order_items` oi -- alias 'oi' for order_items table
  inner join
    `bigquery-public-data.thelook_ecommerce.products` p -- alias 'p' for products table
  on
    oi.product_id = p.id
  where
    oi.status in ('Shipped', 'Processing', 'Complete')
...
-- calculate the profit for each product based on the sales and cost
product_profit as (
  -- subquery to calculate the profit for each product based on the total revenue and cost
  select
    product_id, -- The unique identifier of the product
    category, -- The category of the product
    total_revenue - (product_bought * cost) as profit -- The calculated profit for the product
  from
    product_sales
)

-- final query to get the profit for each product category and month
select
  date_trunc(cast(c.created_at as date), month) as month_year, -- The truncated date to the month for grouping
  b.category, -- The category of the product
  sum(b.profit) as profit -- The total profit for the product category
from
  `bigquery-public-data.thelook_ecommerce.order_items` c -- alias 'c' for order_items table
inner join
  product_profit b -- alias 'b' for product_profit CTE
on
  c.product_id = b.product_id
where
  extract(day from c.created_at) between 1 and 15 -- filter for orders placed between the 1st and 15th day of the month
  and date_trunc(cast(c.created_at as date), month) between '2022-06-01' and '2022-08-01' -- filter for the months of June, July, and August 2022
group by
  1, 2 -- group by month_year and category
order by
  1, 2; -- order the result by month_year in ascending order and category in ascending order
